...
# Escudo de respaldo oficial si no existe el archivo local
...
def inject_modern_css():
    # Nota: No usamos f-string aquí para que las llaves {} de CSS no den error en Python
    st.markdown("""
    <style>
        ...
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        /* Evita bordes molestos en tablas */
...
# --- BASE DE DATOS MAESTRA COMPLETA (138 CADETES) ---
DATOS_GRUPOS_BASE = [
...
def get_processed_guard_for_date(date):
    ...
    for i, c in enumerate(base_group['cadets']):
        # Si hoy salió por Traslado Autorizado, no aparece en su lista original
    ...
    # 2. Integrantes que entran por Traslado Autorizado
    for s in swaps:
    ...
    while curr <= end_date:
        ...
        if pk in st.session_state.punishments and st.session_state.punishments[pk]:
            ...
            for p in st.session_state.punishments[pk]:
                pdf.cell(cols[0], 8, "*", 1, align='C')
                pdf.cell(cols[1], 8, p['nombre'].encode('latin-1', 'replace').decode('latin-1'), 1)
                pdf.cell(cols[2], 8, p['curso'], 1, align='C')
                pdf.cell(cols[3], 8, "REFUERZO", 1, align='C')
                pdf.cell(cols[4], 8, "PRESENTE", 1, align='C')
                pdf.cell(cols[5], 8, "", 1, ln=True)
...
if not st.session_state.logged_in:
    ...
    with st.sidebar:
        ...
        else: st.image(ESCUDO_OFICIAL, width=110)
        st.markdown("<h3 style='color:white; text-align:center; font-size:0.8rem;'>IESP SISTEMA 2026</h3>", unsafe_allow_html=True)
        
        if BASE_URL: st.success("☁️ Sincronización Activa")
        else: st.warning("⚠️ Sin Configuración Cloud")
        ...
        st.divider()
        if st.button("🔄 ACTUALIZAR DATOS", key="sync_sidebar"):
            cloud = load_cloud_data()
    ...
    if menu == "🏠 Dashboard":
        ...
        new_start = st.date_input("Inicio del Ciclo", st.session_state.start_date, key="settings_date_input")
        if st.button("SINCRONIZAR CICLO"):
