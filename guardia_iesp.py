...
# --- ESTILO CSS PROFESIONAL (Corregido sin f-string para evitar SyntaxError) ---
...
if 'initialized' not in st.session_state:
    ...
    if cloud:
        st.session_state.groups = cloud.get("groups", DATOS_GRUPOS_REALES)
        ...
        st.session_state.groups, st.session_state.overrides = DATOS_GRUPOS_REALES, {}
...
if not st.session_state.logged_in:
    ...
    if menu == "🏠 Dashboard":
        ...
        for ex in expired:
            st.markdown(f"<div class='alert-banner'>⚠️ AVISO: El cambio de <b>{ex['cadet_id']}</b> ha caducado. Borrar de 'Cambios Autorizados' para normalizar.</div>", unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown("### ⚠️ Zona de Recuperación")
        st.warning("Si los nombres reales desaparecieron por un error de sincronización, presione el siguiente botón para restaurar la nómina institucional completa (138 cadetes).")
        if st.button("⚡ RESTABLECER NÓMINA INSTITUCIONAL"):
            st.session_state.groups = DATOS_GRUPOS_REALES
            sync_to_cloud()
            st.success("Nómina restaurada con éxito.")
            st.rerun()
