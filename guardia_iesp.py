...
def sync_to_cloud():
    ...
    except: st.error("❌ Falló el guardado.")
...
DATOS_GRUPOS_BASE = [
    ...
    {"id": "G3", "name": "GRUPO N° 3 de II° Año", "cadets": [{"n": 1, "nombre": "Argañaraz Patricio", "curso": "IIIº Año", "funcion": "Jefe de Guardia"}, {"n": 2, "nombre": "Centeno Luis", "curso": "IIIº Año", "funcion": "Cabo de Cuarto"}, {"n": 3, "nombre": "Jiménez Gonzalo", "curso": "IIº Año", "funcion": "Cadete Apostado"}]},
    {"id": "G4", "name": "GRUPO N° 4 de II° Año", "cadets": [{"n": 1, "nombre": "Gramajo Andrea", "curso": "IIIº Año", "funcion": "Jefe de Guardia"}]},
    {"id": "G5", "name": "GRUPO N° 1 de III° Año", "cadets": [{"n": 1, "nombre": "Juarez Ignacio", "curso": "IIIº Año", "funcion": "Jefe de Guardia"}, {"n": 2, "nombre": "Contreras Melani", "curso": "IIIº Año", "funcion": "Cabo de Cuarto"}, {"n": 3, "nombre": "Bareiro Blanca", "curso": "IIº Año", "funcion": "Cadete Apostado"}]},
...
if not st.session_state.logged_in:
    ...
    with header_col2:
        st.markdown("""<h1 style="color:#0f172a; font-weight:800; margin:0; padding-top:10px;">Diagramación Sincronizada</h1>""", unsafe_allow_html=True)
    ...
    if menu == "🏠 Dashboard":
        ...
        st.dataframe(df_display, use_container_width=True, hide_index=True, height=(len(df_display)+1)*35+10, key=f"tbl_{date_key}")
        
        col_m1, col_m2, col_m3 = st.columns(3)
        with col_m1:
            with st.container(border=True):
                st.write("**📝 Asistencia**")
                cad_name = st.selectbox("Personal", [c['nombre'] for c in gi['cadets']], key=f"as_{date_key}")
                nuevo_st = st.selectbox("Estado", ["PRESENTE", "FRANCO", "A.R.T.", "AUSENTE", "NOTA MÉDICA"], key=f"st_{date_key}")
                if st.button("GUARDAR ESTADO", key=f"btn_st_{date_key}"):
        ...
        with col_m2:
            with st.container(border=True):
                st.write("**🎭 Función**")
                cad_name_f = st.selectbox("Personal", [c['nombre'] for c in gi['cadets']], key=f"fu_{date_key}")
                n_f = st.text_input("Nueva Función", key=f"in_f_{date_key}")
                if st.button("ASIGNAR", key=f"btn_f_{date_key}"):
        ...
        with col_m3:
            with st.container(border=True):
                st.write("**🔄 Suplencia**")
                titular_name = st.selectbox("Titular", [c['nombre'] for c in gi['cadets']], key=f"ti_{date_key}")
                all_c = []
                for g in st.session_state.groups:
                    for c in g['cadets']: all_c.append({"label": f"{c['nombre']} ({g['name']})", "obj": c})
                suplente = st.selectbox("Buscar Suplente", range(len(all_c)), format_func=lambda x: all_c[x]['label'], key=f"su_{date_key}")
                if st.button("APLICAR REEMPLAZO", key=f"btn_sup_{date_key}"):
        ...
        with ca:
            with st.container(border=True):
                ...
                sel_c = st.selectbox("Cadete", range(len(all_l)), format_func=lambda x: all_l[x]['label'], key="sw_c")
        ...
        pk = str(st.date_input("Fecha", datetime.now().date(), key="cast_d"))
        ca, cb = st.columns(2)
        with ca:
            with st.container(border=True):
                g_p = st.selectbox("Grupo Origen", range(len(st.session_state.groups)), format_func=lambda x: st.session_state.groups[x]['name'], key="c_g")
                c_p = st.selectbox("Cadete", range(len(st.session_state.groups[g_p]['cadets'])), format_func=lambda x: st.session_state.groups[g_p]['cadets'][x]['nombre'], key="c_c")
        ...
        with cb:
            ...
            if pk in st.session_state.punishments:
                for p in st.session_state.punishments[pk]: st.write(f"• {p['nombre']}")
        ...
        for i, g in enumerate(st.session_state.groups):
            with st.expander(f"Editar {g['name']}"):
                df_red = pd.DataFrame(g['cadets'])
                df_res = st.data_editor(df_red, num_rows="dynamic", key=f"ed_{i}", use_container_width=True)
