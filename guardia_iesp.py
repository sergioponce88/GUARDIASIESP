...
def inject_modern_css():
    ...
    <style>
        ...
        * {{ font-family: 'Plus Jakarta Sans', sans-serif; }}
        ...
        /* Estabilidad para evitar removeChild error en dataframes */
...
def get_cloud_params():
    try:
        ...
        return conf.get("projectId"), st.secrets.get("__app_id", "iesp-v2026-v3")
...
def get_processed_guard_for_date(date):
    ...
    # 2. Traslados autorizados hacia este grupo
    for s in swaps:
...
if not st.session_state.logged_in:
    ...
    with col_login:
        ...
        st.markdown("<h2 style='text-align:center; color:#0f172a;'>SISTEMA DE GUARDIA IESP</h2>", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        st.divider()
        if st.button("🔄 RECARGAR DATOS NUBE", key="btn_sync_cloud"):
            data = load_cloud_data()
            if data: st.session_state.update(data); st.success("Sincronizado"); st.rerun()
        if st.button("🚪 CERRAR SESIÓN", key="btn_logout"): st.session_state.logged_in = False; st.rerun()
    ...
    if menu == "🏠 Dashboard":
        ...
        # KEY DINÁMICA ESTABLE PARA EVITAR REMOVECHILD ERROR
        ...
        with col_m1:
            with st.container(border=True):
                st.write("**📝 Situación**")
                c_idx = st.selectbox("Personal", range(len(gi['cadets'])), format_func=lambda x: gi['cadets'][x]['nombre'], key=f"sel_status_cad_{date_key}")
                nuevo_st = st.selectbox("Estado", ["PRESENTE", "FRANCO", "A.R.T.", "AUSENTE", "NOTA MÉDICA"], key=f"sel_status_val_{date_key}")
                if st.button("ACTUALIZAR ESTADO", key=f"btn_st_save_{date_key}"):
        ...
        with col_m2:
            with st.container(border=True):
                st.write("**🎭 Función**")
                c_idx_f = st.selectbox("Personal", range(len(gi['cadets'])), format_func=lambda x: gi['cadets'][x]['nombre'], key=f"sel_func_cad_{date_key}")
                n_f = st.text_input("Nueva Función", key=f"txt_func_val_{date_key}")
                if st.button("ASIGNAR FUNCIÓN", key=f"btn_func_save_{date_key}"):
        ...
        with col_m3:
            with st.container(border=True):
                ...
                suplente = st.selectbox("Buscar Suplente", range(len(all_c)), format_func=lambda x: all_c[x]['label'], key=f"sel_sup_val_{date_key}")
        ...
        with ca_sw:
            with st.container(border=True):
                ...
                sel_sw = st.selectbox("Cadete a Trasladar", range(len(all_sw_list)), format_func=lambda x: all_swap_list[x]['label'] if 'all_swap_list' in locals() else all_sw_list[x]['label'], key="sw_cad_sel")
        ...
        for i_red, g_red in enumerate(st.session_state.groups):
            with st.expander(f"Editar {g_red['name']}"):
                ...
                if st.button(f"Guardar Estructura {g_red['id']}", key=f"btn_red_save_{i_red}"):
                    st.session_state.groups[i_red]['cadets'] = df_red_res.to_dict('records'); sync_to_cloud(); st.success("Cambio Permanente Guardado")
        ...
        if st.button("RECALIBRAR CICLO", key="btn_settings_save"):
